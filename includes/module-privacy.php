...
function wc_apc_personal_data_exporter($email,$page=1){
    ...
    $codes=[]; foreach($orders as $o){ foreach($o->get_items() as $it){ $val=$it->get_meta(WC_APC_ITEM_META_KEY); if($val){ $codes=array_merge($codes, array_map('trim', explode(',',$val))); } } }
...
function wc_apc_personal_data_eraser($email,$page=1){
    $user=get_user_by('email',$email); $removed=0; if($user){ $orders=wc_get_orders(['customer_id'=>$user->ID,'limit'=>-1,'status'=>array_keys(wc_get_order_statuses()),'return'=>'objects']); $codes=[]; foreach($orders as $o){ foreach($o->get_items() as $it){ $val=$it->get_meta(WC_APC_ITEM_META_KEY); if($val){ $codes=array_merge($codes, array_map('trim', explode(',',$val))); } } } $codes=array_values(array_unique(array_filter($codes))); if($codes){ global $wpdb; $log=$wpdb->prefix.WC_APC_LOG_TABLE; $chunks=array_chunk($codes,100); foreach($chunks as $chunk){ $ph=implode(',',array_fill(0,count($chunk),'%s')); $sql="UPDATE $log SET ip=NULL,user_agent=NULL WHERE code IN ($ph) AND (ip IS NOT NULL OR user_agent IS NOT NULL)"; $wpdb->query($wpdb->prepare($sql,...$chunk)); $removed+=$wpdb->rows_affected; } } }
